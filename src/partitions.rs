...
/// Default offset of the partition table on flash.
pub const DEFAULT_PARTITION_TABLE_OFFSET: u32 = 0x8000;
...
impl AppPartitionSubType {
    pub fn ota_app_number(&self) -> u8 {
        *self as u8 - OTA_SUBTYPE_OFFSET
    }

    pub fn from_ota_app_number(number: u8) -> Result<Self, Error> {
        if number > 16 {
...
pub fn read_partition_table<'a, const PARTITION_TABLE_OFFSET: u32>(
